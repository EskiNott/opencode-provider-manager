...
import {
  APP_VERSION, CACHE_FILE, DETAILS_FILE, MANAGED_FILE, MODELS_DEV_CACHE_FILE, REPORT_FILE,
...
export async function syncProviders({ dryRun = false, listOnly = false, noModelsDev = false, verbose = false, providerFilter = null } = {}) {
  ...
  const target = dryRun ? path.join(path.dirname(new URL(import.meta.url).pathname), "..", "opencode.preview.json") : configFile;
  if (!dryRun) {
